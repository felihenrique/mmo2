...
func (s *Server) Broadcast(event events.ISerializable) {
...
func (s *Server) BroadcastFiltered(event events.ISerializable, filterPeer *gsp.TcpPeer) {
	for _, player := range s.players {
		if player.entity == nil {
			continue
		}
